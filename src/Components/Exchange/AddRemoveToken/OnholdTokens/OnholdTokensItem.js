...
const OnholdTokensItem = ({
  item: { id, token_name, token_symbol, price, supply },
  blockUnblockToken,
}) => {
  const unBlockToken = (e) => {
    e.preventDefault();
    blockUnblockToken({ id, isBlocked: false });
  };
...
export default connect(null, { blockUnblockToken })(OnholdTokensItem);
